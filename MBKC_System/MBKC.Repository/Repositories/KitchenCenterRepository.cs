...
namespace MBKC.Repository.Repositories
{
    public class KitchenCenterRepository
    {
        ...
        public async Task<KitchenCenter> GetKitchenCenterAsync(string managerEmail)
        {
            try
            {
                return await this._dbContext.KitchenCenters.Include(x => x.Manager).FirstOrDefaultAsync(x => x.Manager.Email.Equals(managerEmail) && x.Status != (int)KitchenCenterEnum.Status.DEACTIVE);
            } catch(Exception ex)
            {
                throw new Exception(ex.Message);
            }
