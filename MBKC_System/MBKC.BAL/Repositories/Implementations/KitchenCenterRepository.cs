    ...
    public class KitchenCenterRepository : IKitchenCenterRepository
    {
        ...
        public async Task<GetKitchenCentersResponse> GetKitchenCentersAsync(int? itemsPerPage, int? currentPage, string? searchValue)
        {
            try
            {
                ...
                {
                    numberItems = await this._unitOfWork.KitchenCenterDAO.GetNumberKitchenCentersAsync(searchValue, null);
                    kitchenCenters = await this._unitOfWork.KitchenCenterDAO.GetKitchenCentersAsync(searchValue, null, numberItems, itemsPerPage.Value, currentPage.Value);
                } else if (searchValue != null && StringUtil.IsUnicode(searchValue) == false)
                {
                    numberItems = await this._unitOfWork.KitchenCenterDAO.GetNumberKitchenCentersAsync(null, searchValue);
                    kitchenCenters = await this._unitOfWork.KitchenCenterDAO.GetKitchenCentersAsync(null, searchValue, numberItems, itemsPerPage.Value, currentPage.Value);
                } else if(searchValue == null)
                {
                    numberItems = await this._unitOfWork.KitchenCenterDAO.GetNumberKitchenCentersAsync(null, null);
                    kitchenCenters = await this._unitOfWork.KitchenCenterDAO.GetKitchenCentersAsync(null, null, numberItems, itemsPerPage.Value, currentPage.Value);
                }
        ...
        public async Task UpdateKitchenCenterAsync(int kitchenCenterId, UpdateKitchenCenterRequest updatedKitchenCenter, Email emailOption, FireBaseImage firebaseImageOption)
        {
            ...
            try
            {
                ...
                {
                    Account existedAccount = await this._unitOfWork.AccountDAO.GetAccountAsync(updatedKitchenCenter.ManagerEmail);
                    if(existedAccount != null)
                    {
                        throw new BadRequestException("Manager Email already existed in the system.");
                    }
                    ...
                    this._unitOfWork.AccountDAO.UpdateAccount(existedKitchenCenter.Manager);
                    // tao moi
                    Role kitchenCenterManagerRole = await this._unitOfWork.RoleDAO.GetRoleAsync((int)RoleEnum.Role.KITCHEN_CENTER_MANAGER);
                ...
                this._unitOfWork.KitchenCenterDAO.UpdateKitchenCenter(existedKitchenCenter);
                await this._unitOfWork.CommitAsync();
            }
            catch(NotFoundException ex)
            {
